...
class CarbonIsotopesPage extends Component {
  ...
  componentDidMount() {
    ...
    const el = d3.select(_el)
      .append('svg')
      .attrs(this.props.size);

    let {left, top} = this.props.margin;
    const inner = el.append('g')
    ...
    const b = d3.axisBottom()
      .scale(this.x);

    const xax = inner.append('g')
      .attrs({
        class: 'scale x',
  ...
  componentDidUpdate(prevProps, prevState){
    ...
    let sel = this.dataArea.selectAll('circle')
        .data(this.state.data);

    const path = this.dataArea.selectAll('g.section')
      .data(nested);

    const line = d3.line();

    const esel = path.enter()
      .append('g');
    ...
    esel.append('text')
      .translate(d=> {
        ...
        x: 5,
        y: 5,
        fill(d,i){ return cscale(i); }}).text(d => d.key);
