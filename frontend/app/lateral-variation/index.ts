...
const createVisualization = function(el, units, sections, surfaces){
  ...
  const scaleSize = 100;
  const padding = 500;
  const per_section = Math.floor((size.width-padding-marginX-(2*scaleSize))/(sections.length));
  ...
  const createX = function(d){
    ...
    return double(xv, xv.map(d => d+20));
  };
  ...
  g.append('path')
    .attrs({
      d: areaGenerator,
      fill(d){ return `url(#${d.dominant_lithology})`; },
      'fill-opacity': 0.7,
      stroke: 'transparent'
  });
  ...
  sel.enter()
    ...
    .attrs({
      ...
      fill: 'transparent',
      stroke: 'black',
      'stroke-width'(d){ return d.weight; }
  });
  ...
  g = sel.enter()
    ...
    .attrs({
      class: 'section',
      transform(d){ return `translate(${xloc(d)} ${y(d.end)})`; }
  });
  ...
  g.append("rect")
    .attrs({
      ...
      'stroke-width': 2,
      width: 20,
      height(d){ return y(d.start)-y(d.end); }
  });
  ...
  const names = g.selectAll("text.name")
    .data(sections);
  ...
  names.enter()
    ...
    .attrs({
      class: 'name',
      x: xloc}).text(d => d.section.trim());

  const locales = svg.append('g')
    .attrs({
      class: 'locality',
  ...
  g = sel.enter()
    ...
    .attrs({
      class: 'loc',
      transform(d){ return `translate(${d.x} 0)`; }
  });
  ...
  fm.append('text')
    .attrs({
      class: 'label smaller',
      transform: `translate(${y(350)} -50)`}).text('Formation');
  ...
  sel.enter()
    ...
    .attrs({
      class: 'fm',
      transform(d){ return `translate(${y(d.h)} 0)`; }
  });
...
export default (function(el,cb){
  ...
  return Promise.all([
    ...
    .spread((heights, sections, surfaces) => createVisualization(el, heights, sections, surfaces)).then(() => cb());
});
