...
var makeNested = function(item){
  ...
  const c = item.append('div')
    .attrs({class: style.children});

  const children = c.selectAll('div.child')
    .data(function(d){
      const vals = d.children || [];
      vals.sort((a, b) => a.data.order < b.data.order);
      return vals.filter(d => d.data.level != null);}).enter()
      .append('div')
      .attrs({class(d){
        const ch = d.children || [];
        return classNames(
